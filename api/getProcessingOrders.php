<?php
    ...
    if( $conn ){
        ...
        $datas = json_decode($json);
            ...
            $query = mysqli_query($conn, "SELECT customers.address, customers.location, customers.mobile, customers.pincode, orders.id, orders.processStatus, orders.product, customers.name as customer, transport.name as transport, orders.customerState as customerState, orders.orderedOn, orders.acceptedOn, orders.processedOn FROM `orders`, `customers`, `transport` WHERE orders.customer=customers.id AND orders.transport=transport.id AND orders.customer=customers.id AND orders.status='2'") or die(mysqli_error($conn));			
