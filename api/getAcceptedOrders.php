<?php
    ...
    if( $conn ){
        ...
        $datas = json_decode($json);
            ...
            $query = mysqli_query($conn, "SELECT orders.id, orders.product, customers.name as customer, orders.customerState as customerState, orders.orderedOn, orders.acceptedOn FROM `orders`, `customers` WHERE orders.customer=customers.id AND orders.status='1'") or die(mysqli_error($conn));			
