class Home extends Init{
    ...
    getProcessingOrdersUrl = "./api/getProcessingOrdersForIndividualUser.php";
    async getOrders(){
        let request = await this.apiService({}, "./api/getOrders.php");        
        return request;
    }
    async renderCustomerPayments(){
        let getPaidAmountSoForCustomer = await home.getCustomerPayments(home.getUrlParams('id'))
        .then((result)=>{
            console.log(result);
            let payments = result.paidSoFar.map(payment=>payment.payment);
            let total = payments.reduce((acc, cur)=>{
                return parseFloat(acc)+ parseFloat(cur);
            })
            home.totalPaidAmount = total;              
            return total;                       
        })
        // console.log(getPaidAmountSoForCustomer);
        // return getPaidAmountSoForCustomer;
    }
